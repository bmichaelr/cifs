...
rootFolderBlock * and adds an entry for each folder or file to the registry by hashing the name and adding a registry
...
CIFS_ERROR cifsMountFileSystem(char* cifsFileName)
{
    ...
    for(int i = 0; i < CIFS_SUPERBLOCK_INDEX; i++) {
      unsigned char* buffer = cifsReadBlock(i);
      for(int j = 0; j < CIFS_BLOCK_SIZE; j++) {
        cifsContext->bitvector[j + (i * CIFS_BLOCK_SIZE)] = buffer[j];
      }
      free(buffer);
    }

	// create an in-memory registry of the volume
    cifsContext->registry = malloc(sizeof(void*) * CIFS_REGISTRY_SIZE);

	// TODO: traverse the file system starting with the root and populate the registry

    CIFS_BLOCK_TYPE* root = (CIFS_BLOCK_TYPE*) cifsReadBlock(cifsContext->superblock->cifsRootNodeIndex);
    CIFS_BLOCK_TYPE* rootIndex = (CIFS_BLOCK_TYPE*) cifsReadBlock(cifsContext->superblock->cifsRootNodeIndex);

    // Add the root to the hashtable?
    long index = hash(root->content.fileDescriptor.name);
    addToHashTable(index, root->content.fileDescriptor.name, &root->content.fileDescriptor);
    
    for(int i = 0; i < CIFS_INDEX_SIZE; i++) {
        if(rootIndex->content.index[i] == CIFS_INVALID_INDEX) {
            continue;
        }
        // Some file or folder here
        CIFS_BLOCK_TYPE* file = (CIFS_BLOCK_TYPE*) cifsReadBlock(rootIndex->content.index[i]);
        // TODO: Create a recursive function to go through all folders
        long index = hash(file->content.fileDescriptor.name);
        addToHashTable(index, file->content.fileDescriptor.name, &file->content.fileDescriptor);
    }
...
{
    CIFS_REGISTRY entry = malloc(sizeof(CIFS_REGISTRY_ENTRY_TYPE));
    entry->fileDescriptor = *fd;
    entry->next = NULL;

    if(cifsContext->registry[index] == NULL) {
        cifsContext->registry[index] = entry;
    } else {
        CIFS_REGISTRY tmp = cifsContext->registry[index];
        while(tmp->next != NULL) {
            tmp = tmp->next;
        }
        tmp->next = entry;
